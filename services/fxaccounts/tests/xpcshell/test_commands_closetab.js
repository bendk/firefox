...
add_task(async function test_closetab_send() {
  ...
  const command1 = new RemoteCommand.CloseTab({url: "https://foo.bar/must-send"});
  await store.addRemoteCommandAt(targetDevice.id, command1, now - 15);

  const command2 = new RemoteCommand.CloseTab({url: "https://foo.bar/can-send"});
...
add_task(async function test_closetab_chunking() {
  ...
  for (let i = 0; i < 300; i++) {
    largeNumberOfCommands.push(
      new RemoteCommand.CloseTab({url:
