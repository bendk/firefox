...
use anyhow::Result;
use rinja::Template;
use uniffi_pipeline::{AsRef, Node, NodeConstructMacros};
...
#[derive(Debug, Clone, Node, NodeConstructMacros)]
...
impl<T> CombinedItems<T> {
        ...
        F: FnMut(&mut Module, &mut CombinedItemsIdGenerator, &mut Vec<T>),
    {
        Self::try_new(root, |module, id_generator, items| {
            Ok(f(module, id_generator, items))
        })
        .unwrap()
    }
...
impl FfiFunction {
    pub fn arg_types(&self) -> Vec<&str> {
        self.arguments
            ...
            .chain(self.has_rust_call_status_arg.then(|| "RustCallStatus*"))
            .collect()
    }
}
