...
//
// 32-bits should never overflow and ReleaseAcquire is the recommended memory ordering for reference
// counts:
...
uint64_t CallbackHandleCreate() {
  // The handle will be a pointer to a atomic u32 that stores the ref count.
  // We'll then cast the to a `u64`, which will be the handle.
  //
  // This will safely fit in a JS integer as long as pointers only set the lower 53 bits or so.
  // This is okay since the JS code itself assumes only the lower 48 bits of the pointer are set:
  // https://searchfox.org/firefox-main/rev/20a1fb35a4d5c2f2ea6c865ecebc8e4bee6f86c9/js/public/Value.h#61-66
  //
  // Finally, we always set the lowest bit on the handle.  This allows UniFFI to tell if trait
  // interface handles came from JS or Rust.
...
uint32_t CallbackHandleAddRef(uint64_t aHandle) {
  HandleRefCount* handlePointer = reinterpret_cast<HandleRefCount*>(aHandle & ~1);
