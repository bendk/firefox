...
void FireAndForgetCallbackMethodHandlerBase::ScheduleFireAndForgetCall(
  ...
  nsresult dispatchResult = NS_DispatchToMainThread(NS_NewRunnableFunction(
      "UniFFI callback", [handler = std::move(aHandler),
        ...
        if (error.Failed()) {
          ...
          handler->HandleReturn(aes.cx(), callResult, IgnoreErrors());
          return;
        }
...
void CallbackFreeHandler::MakeCall(JSContext* aCx,
...
void AsyncCallbackMethodHandlerBase::PromiseHandler::ResolvedCallback(
  ...
  mHandler->HandleReturn(aCx, callResult, aRv);
}
